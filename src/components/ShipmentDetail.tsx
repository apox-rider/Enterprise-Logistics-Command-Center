...
export const ShipmentDetail: React.FC<ShipmentDetailProps> = ({
  ...
  return (
    <div onClick={handleclose} className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm flex justify-end animate-fadeIn">
      <div onClick={(e)=>e.stopPropagation()} className={`w-full md:w-1/3 h-screen overflow-y-auto p-6 flex flex-col justify-between shadow-2xl border-l transition-all duration-300 transform translate-x-0 ${isLight ?'bg-white border-slate-200 text-slate-900': 'bg-slate-900 border-slate-800 text-slate-100'}`}>
        <Divide className="border-none ">
          ...
          {toastMessage && (
            <div className={`my-4 p-3 rounded-lg text-xs font-medium border flex items-center gap-2 ${toastMessage.includes('Success') ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
